...
namespace GestionDesAbsencesMigration.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult AddClasses(IFormFile excel)
        {
            ...
            if (FileName != ".xls" && FileName != ".xlsx")
            {
                return RedirectToAction("AllClasses", new { msg = "only excel files are allowed" });
            }
            else
            {
                try
                ...
                catch (Exception e)
                {
                    ...
                    return RedirectToAction("AllClasses", new { msg = "importing failed, error occured" });
                }
            }
            // return View();
        ...
        public ActionResult AddModules(IFormFile excel)
        {
            ...
            if (FileName != ".xls" && FileName != ".xlsx")
            {
                return RedirectToAction("AllModules", new { msg = "only excel files are allowed" });
            }
            else
            {
                try
